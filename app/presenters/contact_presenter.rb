class ContactPresenter < ContentItemPresenter
  ...
  def show_webchat?
    webchat_ids.include?(content_item["base_path"].to_sym)
  end

  def webchat_proxy_url
    "http://localhost:4567/availability/#{webchat_id}"
  end
...
private
  ...
  def webchat_id
    webchat_ids[content_item["base_path"].to_sym]
  end
