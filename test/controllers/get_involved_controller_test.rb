...
class GetInvolvedControllerTest < ActionController::TestCase
  ...
  test "showing total number of open consultations" do
    ...
    stub_request(:get, /\A#{Plek.current.find('search')}\/search.json/)
      .with(query: hash_including(filter_content_store_document_type: "open_consultation"))
  ...
  test "showing total number of closed consultations" do
    ...
    stub_request(:get, /\A#{Plek.current.find('search')}\/search.json/)
      .with(query: hash_including(filter_content_store_document_type: "closed_consultation"))
      .to_return(body: search_response.to_json)
  ...
  test "showing the next closing consultation" do
    Timecop.freeze do
      ...
      search_response = { "results" => [consultation_result(title: title)] }
  ...
  test "showing recent consultation outcomes" do
    ...
    stub_request(:get, /\A#{Plek.current.find('search')}\/search.json/)
      .with(query: hash_including(filter_content_store_document_type: "consultation_outcome"))
      .to_return(body: search_response.to_json)
